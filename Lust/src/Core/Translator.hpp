...
namespace Lust
{
	...
	public:
		...
		static void LoadTranslation(std::string fbd_basepath, Translation* translation);
    private:
		...
		static void TranslationMapToFlatbuffers(std::shared_ptr<RawBuffer>* outputBuffer, const std::unordered_map<std::string, std::unordered_map<std::string, std::u8string>>& data, Language language);
	};
}
