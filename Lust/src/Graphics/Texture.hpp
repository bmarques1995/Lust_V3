...
namespace Lust
{
	...
	public:
		...
		static Texture2D* Instantiate(const GraphicsContext* context, const std::string& path, uint32_t slot, uint32_t spaceSet, uint32_t heapSlot, uint32_t textureIndex);
	};
}
