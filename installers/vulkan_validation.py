...
def main():
    ...
    package_output_dir = "vulkan_validation"
    vvl_dir = append_paths(module_destination, "modules", package_output_dir)
    build_dir = append_paths(module_destination, "dependencies", "windows", package_output_dir)

    clone_and_checkout("https://github.com/KhronosGroup/Vulkan-ValidationLayers.git", destination=vvl_dir, branch=vk_branch)

    run(f'cmake -S "{vvl_dir}" -B "{build_dir}" '
