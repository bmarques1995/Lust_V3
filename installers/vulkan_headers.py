...
def main():
    ...
    package_output_dir = "vulkan_headers"
    vk_headers_dir = append_paths(module_destination, "modules", package_output_dir)
    build_dir = append_paths(module_destination, "dependencies", "windows", package_output_dir)

    clone_and_checkout("https://github.com/KhronosGroup/Vulkan-Headers.git", destination=vk_headers_dir, branch=vk_branch)

    run(f'cmake -S "{vk_headers_dir}" -B "{build_dir}" '
