...
def main():
    ...
    package_output_dir = "libpng"
    libpng_dir = append_paths(module_destination, "modules", package_output_dir)
    build_dir = append_paths(module_destination, "dependencies", "windows", package_output_dir)

    clone_and_checkout("https://git.code.sf.net/p/libpng/code", destination=libpng_dir)

    run(f'cmake -S "{libpng_dir}" -B "{build_dir}" '
