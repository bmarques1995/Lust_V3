...
def main():
    ...
    package_output_dir = "vulkan_extensions"
    vk_extensions_dir = append_paths(module_destination, "modules", package_output_dir)
    build_dir = append_paths(module_destination, "dependencies", "windows", package_output_dir)

    clone_and_checkout("https://github.com/KhronosGroup/Vulkan-ExtensionLayer.git", destination=vk_extensions_dir, branch=vk_branch)

    run(f'cmake -S "{vk_extensions_dir}" -B "{build_dir}" '
